...
export function DeckClient({ cards }: { cards: DeckCard[] }) {
  ...
  if (cards.length === 0) {
    return (
      <section className="rounded-card border border-grid bg-surface p-6 text-center shadow-ambient">
        ...
        <Link
          href="/drill"
          className={cn(button("primary"), "mt-3 inline-block")}
        >
          Go drill →
        </Link>
      </section>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-3">
      ...
      <button
        onClick={advance}
        className="relative block min-h-48 w-full text-left"
      ...
      >
        ...
        >
          <div
            aria-hidden={flipped}
            className="absolute inset-0 rounded-card border border-grid bg-surface p-6 shadow-ambient"
            style={{ backfaceVisibility: "hidden" }}
          >
            <p className="font-mono text-[10px] uppercase tracking-wide text-graphite">
              Trigger cue · {card.subtopicLabel} ·
          ...
          <div
            aria-hidden={!flipped}
            className="absolute inset-0 rounded-card border border-ballpoint/50 bg-highlight p-6 shadow-ambient"
            style={{
