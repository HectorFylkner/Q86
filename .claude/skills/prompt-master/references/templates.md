...
| [E — Chain of Thought](#template-e--chain-of-thought) | Logic tasks on small/non-reasoning models ONLY |
...
| [H — ReAct + Stop Conditions](#template-h--react--stop-conditions) | Legacy/weaker autonomous agents (Devin-class) — NOT frontier agents |
...
**Important:** Only for small and open-weight non-reasoning models (Llama, Mistral, Qwen instruct, mini/nano tiers, older GPT-4-class). Do NOT add CoT to reasoning-native models — Claude 4.5+/Fable 5, GPT-5.x, Gemini 3, o-series, DeepSeek-R1, Qwen thinking mode — they reason internally and CoT instructions degrade output. Control their depth via effort / reasoning_effort / thinking_level instead.
...
**When NOT to use:**
- Any reasoning-native model — Fable 5, GPT-5.x, Gemini 3, o-series, R1, Qwen-thinking (they think internally — adding CoT hurts)
...
*Use for legacy or weaker autonomous agents (Devin, SWE-agent, AutoGPT-class) whose judgment can't be trusted — enumerated boundaries are still the right tool there. Do NOT use this template for frontier agents (Claude Code on current models, Codex): enumerated stop-condition lists degrade them. Use the checkpoint policy, progress grounding, and reversibility-gate modules in references/claude.md §3–4 instead.*
